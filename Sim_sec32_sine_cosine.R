...
signal_data <- function(numx, numxs, size, XY, XSY){
  generate_XL_YL <- function(size, numx, numxs, XY, XSY){
    ...
    mu <- rep(.1, numx) 
    sigma <- matrix(0.5, nrow = numx, ncol = numx)
    diag(sigma) <- 1
    X <- mvrnorm(size, mu, sigma)
    Xstar <- matrix(nrow = size, ncol = numxs)
    for(S in 1:numxs){
      index <- sample(1:numx, numx, replace=FALSE)
      epsilon_Xstar <- mvrnorm(size, rep(0,1), diag(0.1, 1))
      Xstar[,S] <- rowSums(sin(X[,index[1:10]]) + cos(X[,index[11:20]]))/sqrt(20) + epsilon_Xstar
    }
...
multi_study_comp <- function(gene_num, XY, XSY){
  ...
  for(j in 1:300){
    set.seed(j)
    signal   <- signal_data(numx = 20, numxs = 20, size = 300, XY = "High",
                            XSY = "Low")
